...
import com.oo.system.service.ISysDeptService;
...
import com.oo.common.core.text.Convert;
...
import com.oo.system.mapper.SysDeptMapper;
...
public class SysDeptServiceImpl implements ISysDeptService
{
    ...
    @MethodTranslate(dtoClass = SysDept.class)
    ...
    public int insertDept(SysDept dept) {
        ...
        if(dept.getDeptId()==null){
            Long deptId=deptMapper.selectMaxId();
            dept.setDeptId(deptId+1);
        }
        ...
        //保存多语言
        boolean transResult = sysTranslateService.submit(dept.getDeptId().toString(), dept.getTransList());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int deleteDeptByIds(Long[] deptId)
    {
        int result = deptMapper.deleteDeptByIds(deptId);
        if (result <= 0) {
            return result;
        }
        //删除多语言
        List<String> deptIdList = Arrays.stream(deptId).map(Object::toString).collect(Collectors.toList());
        boolean transResult = sysTranslateService.remove(deptIdList);
        return transResult ? result : 0;
    }
    /**
     * 递归列表
