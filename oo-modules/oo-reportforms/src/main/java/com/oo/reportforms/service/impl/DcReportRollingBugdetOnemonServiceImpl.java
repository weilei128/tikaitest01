...
import com.oo.reportforms.mapper.DcReportRollingBugdetOnemonMapper;
import com.oo.reportforms.domain.DcReportRollingBugdetOnemon;
import com.oo.reportforms.service.DcReportRollingBugdetOnemonService;

import javax.servlet.http.HttpServletResponse;

/**
 * 滚动预测1+11Service业务层处理
 * 
 * @author oo
 * @date 2023-10-31
 */
@Service
public class DcReportRollingBugdetOnemonServiceImpl extends ServiceImpl<DcReportRollingBugdetOnemonMapper, DcReportRollingBugdetOnemon> implements DcReportRollingBugdetOnemonService
{
    @Autowired(required = false)
    private DcReportRollingBugdetOnemonMapper dcReportRollingBugdetOnemonMapper;
    @Autowired(required = false)
    private DcReportDrillWellAssesMapper dcReportDrillWellAssesMapper;
    @Autowired(required = false)
    private DcReportRollingBugdetOnemonMonthService dcReportRollingBugdetOnemonMonthService;
    /**
     * 查询滚动预测1+11列表
     * 
    ...
    {
        return baseMapper.selectDcReportRollingBugdetOnemonList(dcReportRollingBugdetOnemon);
    }
    /**
     * 查询
     */
    ...
    public List<DcReportRollingBugdetOnemonVo> selectList(String organizationId, String year, String budgetSubjects) throws Exception{
        QueryWrapper<DcReportRollingBugdetOnemon> queryWrapper = new QueryWrapper<>();
        ...
        List<DcReportRollingBugdetOnemon> dcReportRollingBugdetOnemonList = dcReportRollingBugdetOnemonMapper.selectList(queryWrapper);
        List<Map<String, Object>> maps = new ArrayList<>();
        Map<String, Object> map = new HashMap<String, Object>();
        DcReportRollingBugdetOnemonVo book = new DcReportRollingBugdetOnemonVo();
        List<DcReportRollingBugdetOnemonVo> dcReportRollingBugdetOnemonVoList = new ArrayList<>();
        String dictLabel ="";

        for (DcReportRollingBugdetOnemon dcReportRollingBugdetOnemon : dcReportRollingBugdetOnemonList) {
            List<DcReportRollingBugdetOnemonMonth> dcReportRollingBugdetOnemonMonthList = dcReportRollingBugdetOnemonMapper.selectDcReportRollingBugdetOnemonYear(dcReportRollingBugdetOnemon.getId());
            ...
            if (dcReportRollingBugdetOnemonMonthList.size() > 0) {
                maps = RowConvertColUtil.doConvertReturnObj(dcReportRollingBugdetOnemonMonthList, "month", fixedColumn, "value", true, fixedColumnName, null).getDataList();
                book = JSON.parseObject(JSON.toJSONString(maps.get(0)), DcReportRollingBugdetOnemonVo.class);
            }//map转DcReportOutputWellBugdetVo实体类
            BeanUtils.copyProperties(dcReportRollingBugdetOnemon, book);
            if (StringUtils.isNotEmpty(dcReportRollingBugdetOnemon.getOrganizationId())) {
                book.setOrganizationName(dcReportDrillWellAssesMapper.selectName((dcReportRollingBugdetOnemon.getOrganizationId())));
            }
            if (StringUtils.isNotEmpty(dcReportRollingBugdetOnemon.getType())) {
                dictLabel = DictUtils.getDictLabel("dc_pk_well_type", dcReportRollingBugdetOnemon.getType());
                book.setType(dictLabel == null ? "" : dictLabel);
                if (StringUtils.isNotEmpty(dcReportRollingBugdetOnemon.getBudgetSubjects())) {
                    dictLabel = DictUtils.getDictLabel("well_config", dcReportRollingBugdetOnemon.getBudgetSubjects());
                    book.setBudgetSubjects(dictLabel == null ? "" : dictLabel);
                }
            }
            dcReportRollingBugdetOnemonVoList.add(book);
        }
        return dcReportRollingBugdetOnemonVoList;
    }
     ...
     */
    @Override
    public String add(List<DcReportRollingBugdetOnemonVo> dcReportRollingBugdetOnemonVos) throws Exception{
        for(DcReportRollingBugdetOnemonVo dcReportRollingBugdetOnemonVo:dcReportRollingBugdetOnemonVos){
            if("0".equals(save1(dcReportRollingBugdetOnemonVo))){
                return "保存/更新失败";
            }
        }
        return "保存/更新成功";
    ...
    @DictMethod(dtoClass = DcReportRollingBugdetOnemonVo.class)
    public String save1(DcReportRollingBugdetOnemonVo dcReportRollingBugdetOnemonVo) {
        ...
        if (StringUtils.isEmpty(dcReportRollingBugdetOnemonVo.getId())) {
            UUID uuid = UUID.randomUUID();
            String id = uuid.toString().replace("-","");
            dcReportRollingBugdetOnemonVo.setId(id);
        }
        DcReportRollingBugdetOnemon dcReportRollingBugdetOnemon = new DcReportRollingBugdetOnemon();
        BeanUtils.copyProperties(dcReportRollingBugdetOnemonVo, dcReportRollingBugdetFivemonVo2);
        BeanUtils.copyProperties(dcReportRollingBugdetOnemonVo, dcReportRollingBugdetOnemon);

        dcReportRollingBugdetOnemon.setYear(DateUtils.getDate().substring(0,4));

        if(!this.saveOrUpdate(dcReportRollingBugdetOnemon)){
            return "0";
        }

        RowConvertColUtil.coverObjToListRollingBugdet(dcReportRollingBugdetOnemonVo.getId(), dcReportRollingBugdetFivemonVo2, arraryList);
        if(!dcReportRollingBugdetOnemonMonthService.saveOrUpdateBatchByMultiId(arraryList)){
            return "0";
        }
        return "1";
    }
    ...
    public void export(HttpServletResponse response,String organizationId, String year,String budgetSubjects) throws Exception {
        // excel模板路径
        String templateName = "（计划费控岗03）滚动预测1+11VM0(导出).xlsx";
        XSSFWorkbook wb = ReportUtils.getTemplate(templateName);
        List<DcReportRollingBugdetOnemonVo> dcReportRollingBugdetOnemonVoList = selectList(organizationId,year,budgetSubjects);
        ...
        titleRow.getCell(6).setCellValue(String.valueOf(years) + " 1+11滚动预测（预算）");
        titleRow.getCell(20).setCellValue(String.valueOf(years) + "工作量");
        titleRow.getCell(21).setCellValue(String.valueOf(years) + " 1+11滚动预测（工作量）");


        for (DcReportRollingBugdetOnemonVo item : dcReportRollingBugdetOnemonVoList) {
    ...
    public String importData(List<DcReportRollingBugdetOnemonVo> dataList){
        ...
        for (DcReportRollingBugdetOnemonVo item : dataList){
