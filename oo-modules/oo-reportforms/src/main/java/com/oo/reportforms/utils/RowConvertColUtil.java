...
public class RowConvertColUtil {
     ...
     * @param arraryList
     */
    public static void coverObjToList(String id,Object obj, List<DcReportOutputWellBudgetMonth> arraryList) {
        if (ObjectUtil.isEmpty(obj)){
            return;
        }
        Field[] declaredFields = obj.getClass().getDeclaredFields();
        Integer start = 0;
        Integer end = declaredFields.length-1;
        try {
            try {
                for (int i = start; i <= end; i++){
                    DcReportOutputWellBudgetMonth dcReportOutputWellBudgetMonth=new DcReportOutputWellBudgetMonth();
                    Field f = obj.getClass().getDeclaredField(declaredFields[i].getName());
                    String month=declaredFields[i].getName();
                    // 在反射时允许访问私有变量
                    ...
                    if(o==null){
                        dcReportOutputWellBudgetMonth.setValue(null);
                    }else{
                        dcReportOutputWellBudgetMonth.setValue(new BigDecimal(decimalFormat.format(o)));
                    }
                    dcReportOutputWellBudgetMonth.setMonth(month);
                    dcReportOutputWellBudgetMonth.setOutputWellId(id);
                    arraryList.add(dcReportOutputWellBudgetMonth);
                }
     ...
     * @param arraryList
     */
    public static void coverObjToList1(String id,Object obj, List<DcReportAssetBudgetMonth> arraryList) {
        if (ObjectUtil.isEmpty(obj)){
            return;
        }
        Field[] declaredFields = obj.getClass().getDeclaredFields();
        ...
        try {
            try {
                for (int i = start; i <= end; i++){
                    DcReportAssetBudgetMonth dcReportAssetBudgetMonth=new DcReportAssetBudgetMonth();
                    Field f = obj.getClass().getDeclaredField(declaredFields[i].getName());
                    String month=declaredFields[i].getName();
                    // 在反射时允许访问私有变量
                    f.setAccessible(true);
                    ...
                    DecimalFormat decimalFormat = new DecimalFormat("0.00");
                    if(o==null){
                        dcReportAssetBudgetMonth.setValue(null);
                    }else{
                        dcReportAssetBudgetMonth.setValue(new BigDecimal(decimalFormat.format(o)));
                    }
                    dcReportAssetBudgetMonth.setMonth(month);
                    dcReportAssetBudgetMonth.setAssetId(id);
                    arraryList.add(dcReportAssetBudgetMonth);
                }
            } catch (NoSuchFieldException e) {
                e.printStackTrace();
            } catch (IllegalArgumentException e) {
     ...
     * @param arraryList
     */
    public static void coverObjToListRollingBugdet(String id,Object obj, List<DcReportRollingBugdetOnemonMonth> arraryList) {
        if (ObjectUtil.isEmpty(obj)){
            return;
        }
        Field[] declaredFields = obj.getClass().getDeclaredFields();
        ...
        try {
            try {
                for (int i = start; i <= end; i++){
                    DcReportRollingBugdetOnemonMonth dcReportRollingBugdetOnemonMonth=new DcReportRollingBugdetOnemonMonth();
                    Field f = obj.getClass().getDeclaredField(declaredFields[i].getName());
                    String month=declaredFields[i].getName();
                    // 在反射时允许访问私有变量
                    f.setAccessible(true);
                    ...
                    DecimalFormat decimalFormat = new DecimalFormat("0.00");
                    if(o==null){
                        dcReportRollingBugdetOnemonMonth.setValue(null);
                    }else{
                        dcReportRollingBugdetOnemonMonth.setValue(new BigDecimal(decimalFormat.format(o)));
                    }
                    dcReportRollingBugdetOnemonMonth.setMonth(month);
                    dcReportRollingBugdetOnemonMonth.setRollingBugdetId(id);
                    arraryList.add(dcReportRollingBugdetOnemonMonth);
                }
            } catch (NoSuchFieldException e) {
                e.printStackTrace();
            } catch (IllegalArgumentException e) {
