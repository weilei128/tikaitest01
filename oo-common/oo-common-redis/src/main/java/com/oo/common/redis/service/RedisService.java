package com.oo.common.redis.service;
...
public class RedisService
{
     ...
     * @return 结果
     */
    public boolean hasKeyPrefix(String keyPrefix) {
        if (keyPrefix != null) {
            Set<String> keys = redisTemplate.keys(Pattern.matches("\\*$", keyPrefix) ? keyPrefix : keyPrefix + "*");
     ...
     * @return 结果
     */
    public <T> List<T> getCacheListByPrefix(String keyPrefix) {
        if (keyPrefix != null) {
            Set<String> keys = redisTemplate.keys(Pattern.matches("\\*$", keyPrefix) ? keyPrefix : keyPrefix + "*");
            if(CollectionUtils.isNotEmpty(keys)) {
                List<T> result = new ArrayList<>();
                for(String key : keys) {
                    result.addAll(getCacheList(key));
                }
                return result;
            }
        }
